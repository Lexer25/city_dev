...
<div class="panel panel-primary">
	...
	echo '<th class="filter-select" data-placeholder="Все">'.__('isAlarm').'</th>';
            ...
            foreach ($list as $key => $deviceInfo) {
                ...
                // Колонка 7 - isWp
                if($deviceInfo->onLine) {
                    echo '<td>'.Form::checkbox('', 1, $deviceInfo->isWP == true, array('disabled'=>'disabled')).'<span class="hidden">'.($deviceInfo->isWP ? '1' : '0').'</span></td>';
                } else {
                    echo '<td>-</td>';
                }
                ...
                // Колонка 8 - isTest
                if($deviceInfo->onLine) {
                    echo '<td>'.Form::checkbox('', 1, $deviceInfo->isTest == true, array('disabled'=>'disabled')).'<span class="hidden">'.($deviceInfo->isTest ? '1' : '0').'</span></td>';
                } else {
                    echo '<td>-</td>';
                }
